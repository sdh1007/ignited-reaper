...
export function OptimizedAtmosphere({ qualityTier: initialQuality = 'high' }: OptimizedAtmosphereProps) {
  const { isDayMode } = useCemeteryStore()
  const [prefersReducedMotion, setPrefersReducedMotion] = useState(false)
  const [dynamicQuality, setDynamicQuality] = useState<'high' | 'medium' | 'low'>(initialQuality)
  ...
  const particleUpdateRef = useRef(0)
  ...
  const isMedium = dynamicQuality === 'medium'
  const isLow = dynamicQuality === 'low'
  ...
  }, [prefersReducedMotion, isMedium, isLow])
  ...
  // Instanced particle configurations
  const fireflies = useMemo<ParticleConfig[]>(() => {
    const baseCount = prefersReducedMotion ? 16 : isLow ? 18 : isMedium ? 28 : 40
  ...
  }, [prefersReducedMotion, isMedium, isLow, particleMultiplier])

  const embers = useMemo<ParticleConfig[]>(() => {
    const baseCount = prefersReducedMotion ? 6 : isLow ? 8 : isMedium ? 12 : 20
    const count = Math.floor(baseCount * particleMultiplier)
    return Array.from({ length: count }, (_, i) => ({
      id: i,
  ...
  const dustMotes = useMemo<ParticleConfig[]>(() => {
    const baseCount = prefersReducedMotion ? 12 : 25
    const count = Math.floor(baseCount * particleMultiplier)
    return Array.from({ length: count }, (_, i) => ({
      id: i,
  ...
  }, [prefersReducedMotion, particleMultiplier])
  ...
  useEffect(() => {
    if (!fireflyMeshRef.current || isDayMode || fireflies.length === 0) return
    
    try {
      fireflyMeshRef.current.instanceMatrix.setUsage(THREE.DynamicDrawUsage)

      fireflies.forEach((config, index) => {
        particleDummy.position.set(config.startPos[0], config.startPos[1], config.startPos[2])
        particleDummy.scale.setScalar(config.size)
        particleDummy.updateMatrix()
        fireflyMeshRef.current?.setMatrixAt(index, particleDummy.matrix)
      })
      fireflyMeshRef.current.instanceMatrix.needsUpdate = true
    } catch (error) {
      console.warn('Failed to initialize firefly instances:', error)
    }
  }, [fireflies, particleDummy, isDayMode])

  // Initialize embers
  useEffect(() => {
    if (!emberMeshRef.current || isDayMode || embers.length === 0) return
    
    try {
      emberMeshRef.current.instanceMatrix.setUsage(THREE.DynamicDrawUsage)

      embers.forEach((config, index) => {
        particleDummy.position.set(config.startPos[0], config.startPos[1], config.startPos[2])
        particleDummy.scale.setScalar(config.size)
        particleDummy.updateMatrix()
        emberMeshRef.current?.setMatrixAt(index, particleDummy.matrix)
      })
      emberMeshRef.current.instanceMatrix.needsUpdate = true
    } catch (error) {
      console.warn('Failed to initialize ember instances:', error)
    }
  }, [embers, particleDummy, isDayMode])

  // Initialize dust motes
  useEffect(() => {
    if (!dustMeshRef.current || !isDayMode || dustMotes.length === 0) return
    
    try {
      dustMeshRef.current.instanceMatrix.setUsage(THREE.DynamicDrawUsage)

      dustMotes.forEach((config, index) => {
        particleDummy.position.set(config.startPos[0], config.startPos[1], config.startPos[2])
        particleDummy.scale.setScalar(config.size)
        particleDummy.updateMatrix()
        dustMeshRef.current?.setMatrixAt(index, particleDummy.matrix)
      })
      dustMeshRef.current.instanceMatrix.needsUpdate = true
    } catch (error) {
      console.warn('Failed to initialize dust mote instances:', error)
    }
  }, [dustMotes, particleDummy, isDayMode])

  useFrame((state, delta) => {
    // Performance monitoring
    perfMonitor.measureFPS()
    if (perfMonitor.shouldCheck()) {
      const newQuality = perfMonitor.getQualityTier()
      const newMultiplier = perfMonitor.getParticleMultiplier()
      
      if (newQuality !== dynamicQuality) {
        setDynamicQuality(newQuality)
      }
      if (Math.abs(newMultiplier - particleMultiplier) > 0.1) {
    ...
    particleUpdateRef.current += delta
    ...
    if (!isDayMode && starMeshRef.current && starConfigs.length > 0 && starUpdateRef.current >= (prefersReducedMotion ? 1 / 28 : isLow ? 1 / 32 : isMedium ? 1 / 38 : 1 / 45)) {
      ...
      try {
        ...
        starConfigs.forEach((config, index) => {
          const twinkle = config.baseScale + Math.sin(t * config.speed + config.phase) * twinkleAmplitude
          starDummy.position.set(config.position[0], config.position[1], config.position[2])
    ...
    // Animate fireflies (instanced)
    if (!isDayMode && fireflyMeshRef.current && fireflies.length > 0 && particleUpdateRef.current >= (prefersReducedMotion ? 1 / 40 : isLow ? 1 / 36 : isMedium ? 1 / 45 : 1 / 55)) {
      try {
        fireflies.forEach((config, i) => {
          const time = t * config.speed + config.phase
          const motionScale = prefersReducedMotion ? 0.5 : 1

          // Complex flight pattern
          const x = config.startPos[0] + (Math.sin(time * 0.7) * 3 + Math.cos(time * 0.3) * 1.5) * motionScale
          const y = config.startPos[1] + (Math.sin(time * 0.5) * 2 + Math.cos(time * 0.8) * 0.8) * motionScale
          const z = config.startPos[2] + (Math.cos(time * 0.6) * 3 + Math.sin(time * 0.4) * 1.2) * motionScale

          // Keep fireflies in bounds
          const boundedX = Math.abs(x) > 25 ? x * -0.1 : x
          const boundedZ = Math.abs(z) > 25 ? z * -0.1 : z
          const boundedY = y > 10 ? 1 : y < 0.5 ? 8 : y

          particleDummy.position.set(boundedX, boundedY, boundedZ)
          particleDummy.scale.setScalar(config.size)
          particleDummy.updateMatrix()
          fireflyMeshRef.current?.setMatrixAt(i, particleDummy.matrix)
        })
        fireflyMeshRef.current.instanceMatrix.needsUpdate = true
      } catch (error) {
        // Silently skip frame if there's an error
    ...
    // Animate embers (instanced)
    if (!isDayMode && emberMeshRef.current && embers.length > 0 && particleUpdateRef.current >= (prefersReducedMotion ? 1 / 36 : isLow ? 1 / 34 : isMedium ? 1 / 38 : 1 / 50)) {
      particleUpdateRef.current = 0
      
      try {
        embers.forEach((config, i) => {
          const time = t * config.speed + config.phase
          const swayScale = prefersReducedMotion ? 0.5 : 1
          
          const x = config.startPos[0] + Math.sin(time * 0.5) * 0.5 * swayScale
          let y = config.startPos[1] + time * (prefersReducedMotion ? 0.3 : 0.5)
          const z = config.startPos[2] + Math.cos(time * 0.3) * 0.3 * swayScale

          // Reset when too high
          if (y > 15) {
            y = 0
            config.startPos[0] = (Math.random() - 0.5) * 40
            config.startPos[2] = (Math.random() - 0.5) * 40
          }

          particleDummy.position.set(x, y, z)
          particleDummy.scale.setScalar(config.size)
          particleDummy.updateMatrix()
          emberMeshRef.current?.setMatrixAt(i, particleDummy.matrix)
        })
        emberMeshRef.current.instanceMatrix.needsUpdate = true
      } catch (error) {
        // Silently skip frame if there's an error
    ...
    // Animate dust motes (instanced)
    if (isDayMode && dustMeshRef.current && dustMotes.length > 0 && particleUpdateRef.current >= (prefersReducedMotion ? 1 / 30 : 1 / 45)) {
      try {
        dustMotes.forEach((config, i) => {
          const time = t * config.speed + config.phase
          
          const x = config.startPos[0] + Math.sin(time * 0.3) * 0.3
          const y = config.startPos[1] + Math.sin(time * 0.2) * 0.2
          const z = config.startPos[2] + Math.cos(time * 0.25) * 0.25

          particleDummy.position.set(x, y, z)
          particleDummy.scale.setScalar(config.size)
          particleDummy.updateMatrix()
          dustMeshRef.current?.setMatrixAt(i, particleDummy.matrix)
        })
        dustMeshRef.current.instanceMatrix.needsUpdate = true
      } catch (error) {
        // Silently skip frame if there's an error
  ...
  return (
    <group>
      ...
      {/* Instanced fireflies for night mode */}
      {!isDayMode && fireflies.length > 0 && (
        <instancedMesh
          ref={fireflyMeshRef}
      ...
      {/* Instanced ember particles for night mode */}
      {!isDayMode && embers.length > 0 && (
        <instancedMesh
          ref={emberMeshRef}
      ...
      {/* Instanced floating dust motes for day mode */}
      {isDayMode && dustMotes.length > 0 && (
        <instancedMesh
          ref={dustMeshRef}
          args={[undefined, undefined, dustMotes.length]}
          frustumCulled={true}
        >
