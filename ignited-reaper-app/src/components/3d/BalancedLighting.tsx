...
interface BalancedLightingProps {
  qualityTier: 'high' | 'medium' | 'low'
}

export function BalancedLighting({ qualityTier }: BalancedLightingProps) {
  ...
  // Strategic candle positions for warm lighting contrast
  ...
  return (
    <group>
      ...
      {/* Additional atmospheric lighting for depth */}
      <pointLight position={[0, 8, 0]} intensity={0.3} color="#f0f4f8" distance={30} decay={2} />
      <pointLight position={[-20, 6, -20]} intensity={0.2} color="#e2e8f0" distance={25} decay={2.5} />
      <pointLight position={[20, 6, 20]} intensity={0.2} color="#e2e8f0" distance={25} decay={2.5} />
    </group>
  )
}
